...
void reci_enc_bwd_v2(
    ...
    for (long long int e = e_start; e < e_end; e++)
    {
        ...
        for (float n1 =  0, s1=0; n1 <= R; n1++, s +=s1, s1=0) { float scale = n1==0.0f ? 1.0f : 2.0f;
        for (float n2 = -R, s2=0; n2 <= R; n2++, s1+=s2, s2=0)
        for (float n3 = -R;       n3 <= R; n3++)
        {
            ...
            s2 += kk * expf(a*kk) * cosf(kr_base_1*n1 + kr_base_2*n2 + kr_base_3*n3) * scale;
        }
        }
